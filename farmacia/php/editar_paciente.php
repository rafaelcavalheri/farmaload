...
// Recebe o ID do paciente para edição
$pacienteId = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
if (!$pacienteId) {
    die("ID de paciente inválido.");
}

// Carregar dados do paciente para mostrar no formulário
...
$stmt = $pdo->prepare("SELECT * FROM pessoas_autorizadas WHERE paciente_id = ? ORDER BY id LIMIT 3");
$stmt->execute([$pacienteId]);
$pessoas_autorizadas = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Preenche array com 3 posições, mesmo que vazias
$autorizados = array_pad($pessoas_autorizadas, 3, ['nome' => '', 'cpf' => '']);

// Se veio POST, usa os dados do POST, senão usa os dados do banco
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !validarTokenCsrf($_POST['csrf_token'])) {
        die("Token CSRF inválido!");
    }

    // Debug para verificar os valores recebidos do POST
    ...
    if (!empty($valores['codigo_paciente']) && strlen($valores['codigo_paciente']) > 50) {
        $erros['codigo_paciente'] = 'Código do paciente deve ter no máximo 50 caracteres.';
    }
    ...
    if (isset($_POST['medicamento_id']) && is_array($_POST['medicamento_id'])) {
        ...
        foreach ($_POST['medicamento_id'] as $i => $medId) {
            ...
            $renovado = isset($_POST['renovado'][$medId]) ? 1 : 0;
                    ...
                    if (!empty($medico_id)) {
                        // Verificar se é médico ou instituição
                        if ($medico_id >= 10000) {
    ...
    if (empty($erros)) {
        try {
            ...
            // Converte a data de renovação para o formato do banco (YYYY-MM-DD)
            $validade = null;
            if (!empty($_POST['renovacao'][0])) {
                $dataRenovacao = $_POST['renovacao'][0];
                if (preg_match('/^(\d{2})\/(\d{2})\/(\d{4})$/', $dataRenovacao, $matches)) {
                    $dia = $matches[1];
                    $mes = $matches[2];
                    $ano = $matches[3];
                    $validade = sprintf('%s-%s-%s', $ano, $mes, $dia);
                }
            } else if (!empty($paciente['validade'])) {
                // Se não foi fornecida nova data, mantém a data atual
                $validade = $paciente['validade'];
            }

            // Atualiza paciente
            $stmt = $pdo->prepare("UPDATE pacientes SET nome = ?, codigo_paciente = ?, cpf = ?, sim = ?, nascimento = ?, telefone = ?, telefone2 = ?, observacao = ?, validade = ?, renovado = ? WHERE id = ?");
            $stmt->execute([
                $valores['nome'],
                ...
                $valores['observacao'],
                $validade,
                isset($_POST['renovado'][0]) ? 1 : 0,
                $pacienteId
            ]);

            // Remove pessoas autorizadas antigas
            $stmt = $pdo->prepare("DELETE FROM pessoas_autorizadas WHERE paciente_id = ?");
            $stmt->execute([$pacienteId]);

            // Insere novas pessoas autorizadas
            ...
            if (!empty($valores['medicamentos'])) {
                ...
                // Preparar statements
                $stmtUpdate = $pdo->prepare("UPDATE paciente_medicamentos SET nome_medicamento = ?, quantidade = ?, quantidade_solicitada = ?, cid = ?, observacoes = ?, medico_id = ?, tipo_prescritor = ?, instituicao_id = ?, renovacao = ?, renovado = ?, codigo_paciente = ? WHERE id = ?");
                $stmtInsert = $pdo->prepare("INSERT INTO paciente_medicamentos (paciente_id, codigo_paciente, medicamento_id, nome_medicamento, quantidade, quantidade_solicitada, cid, observacoes, medico_id, tipo_prescritor, instituicao_id, renovacao, renovado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $valores['nome'] = $paciente['nome'];
    $valores['codigo_paciente'] = $paciente['codigo_paciente'] ?? '';
    $valores['cpf'] = $paciente['cpf'];
    $valores['sim'] = $paciente['sim'];
    $valores['nascimento'] = $paciente['nascimento'];
    $valores['telefone'] = $paciente['telefone'];
    $valores['telefone2'] = $paciente['telefone2'];
    $valores['observacao'] = $paciente['observacao'];
    $valores['validade'] = $paciente['validade'];

    // Buscar medicamentos associados
    $stmtMed = $pdo->prepare("
        SELECT pm.*, m.nome as medicamento_nome, pm.renovado, pm.medicamento_id,
               pm.tipo_prescritor, pm.instituicao_id, pm.observacoes,
               med.nome as medico_nome, inst.nome as instituicao_nome
        FROM paciente_medicamentos pm 
        JOIN medicamentos m ON pm.medicamento_id = m.id 
        LEFT JOIN medicos med ON pm.medico_id = med.id
        LEFT JOIN instituicoes inst ON pm.instituicao_id = inst.id
    ...
    <fieldset>
        ...
        <fieldset>
            ...
            <div class="button-container">
                <button type="button" id="btn-add-medicamento"><i class="fas fa-plus"></i> Medicamento</button>
            </div>
        </fieldset>
