...
// Inclui funções de detecção de dispositivos móveis
...
// Processa redirecionamento para dispositivos móveis (apenas para requisições não-API)
if (!$isApi) {
...
if ($isApi) {
    ...
    // CORS mais restritivo - permitir apenas domínios específicos
    $allowedOrigins = [
        'http://localhost',
        'http://localhost:9010',
        'http://127.0.0.1',
        'http://127.0.0.1:9010',
        'https://farmalto.mogimirim.sp.gov.br'
    ];
...
function authenticateADUser($email, $senha, $ldapServer, $ldapDomain, $ldapBaseDn) {
    ...
    // Validação de senha vazia - CORREÇÃO PRINCIPAL
    if (empty($senha) || trim($senha) === '') {
        error_log("Senha vazia fornecida para usuário LDAP: " . $email);
        return false;
    }
...
if ($isApi) {
    ...
    if (!verificarTentativasLogin($email)) {
        echo json_encode(['success' => false, 'message' => 'Muitas tentativas de login. Tente novamente em 1 hora.']);
        exit;
    }
    ...
    try {
        if ($loginType === 'ldap') {
            ...
            // Autenticação local
            $stmt = $pdo->prepare("SELECT * FROM usuarios WHERE (email = ? OR email = CONCAT(?, '@farmacia.local')) AND auth_type = 'local'");
